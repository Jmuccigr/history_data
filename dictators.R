...
for (gap in c(50)) {
    endYr <- endYr + (startYr - endYr) %% gap
    colorCt <- round((endYr - startYr) / gap)
    allcolors <-
        c(gray.colors(colorCt),
          plasma(colorCt),
          rep("white", colorCt),
          rep("black", colorCt))
    for (i in 1:(length(allcolors) / colorCt)) {
        ...
        p <- ggplot(dictators, aes(x = Year)) +
            ...
            labs(
                title = "Occurrence of Dictators during the Roman Republic",
                x = "Year BCE",
                y = "Count",
                caption = "CC BY-NC 4.0 John D. Muccigrosso"
            ) +
            scale_y_continuous(breaks = seq(0, 100, 5), minor_breaks = c()) +
            scale_x_continuous(breaks = seq(startYr,endYr,gap), minor_breaks = c()) +
            annotate(
                "text",
                x = -60,
                hjust = 1,
            ...
            geom_histogram(
                breaks = seq(startYr, endYr, gap),
                color = bcol[i],
                fill = allcolors[startColor:endColor]
            ) +
            xlim(c(startYr - gap, endYr - gap))
        #svg(paste("dictators_", gap, "_", allcolorsList[i], ".svg", sep = ""))
        plot(p)
        #dev.off()
    }
}
